...
class VIB(nn.Module):
    ...
    def forward(self, feature_list, x):
        ...
        return total_loss
class SimplifiedWpLoss(nn.Module):
    def __init__(self, device):
        super(SimplifiedWpLoss, self).__init__()
        self.device = device
...
class PlanKD(nn.Module):
    def __init__(
        ...
        self.wp_lstm_loss = SimplifiedWpLoss(device=self.device)  # 使用新的 LSTM Loss 类
